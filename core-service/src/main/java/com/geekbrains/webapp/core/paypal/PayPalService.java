package com.geekbrains.webapp.core.paypal;
...
public class PayPalService {
    ...
    public OrderRequest createOrderRequest(Long orderId) {
        com.geekbrains.webapp.core.model.Order order = orderService.findById(orderId).orElseThrow(() -> new ResourceNotFoundException("Order not found"));
        ...
        PurchaseUnitRequest purchaseUnitRequest = new PurchaseUnitRequest()
                ...
                .amountWithBreakdown(new AmountWithBreakdown().currencyCode("RUB").value(String.valueOf(order.getPrice()))
                        .amountBreakdown(new AmountBreakdown().itemTotal(new Money().currencyCode("RUB").value(order.getPrice().toPlainString()))))
                .items(order.getItems().stream()
                        .map(orderItem -> new Item()
                                .name(orderItem.getProduct().getTitle())
                                .unitAmount(new Money().currencyCode("RUB").value(order.getPrice().toPlainString()))
